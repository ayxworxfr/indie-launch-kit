...
├── src/
│   ├── content/
│   │   └── pages/               # ★ 用户写的 Markdown 内容（自动生成路由和导航）
│   │       ├── privacy-zh.md    # 命名规则：{slug}-{lang}.md
│   │       ├── privacy-en.md    # 对应路由：/{lang}/{slug}
│   │       ├── terms-zh.md      # 新增文件即自动生成新页面和导航链接
│   │       └── terms-en.md      # frontmatter 必须包含 title 字段
│   │
...
│   │   ├── Header.astro         # 顶部导航（Logo + 动态页面链接 + 语言切换）
...
│   │   └── [lang]/
│   │       ├── index.astro      # 首页
│   │       └── [slug].astro     # 动态内容页（自动枚举 content/pages/ 生成路由）
│   │
│   ├── styles/
...
/zh/           → 中文首页
/zh/{slug}     → 中文内容页（由 content/pages/{slug}-zh.md 自动生成）
/en/           → 英文首页
/en/{slug}     → 英文内容页（由 content/pages/{slug}-en.md 自动生成）
```

Astro 内置 i18n 路由，`prefixDefaultLocale: true` 确保两种语言 URL 格式一致。

`src/pages/[lang]/[slug].astro` 在构建时调用 `getCollection('pages')` 枚举所有 Markdown 文件，按 `{slug}-{lang}` 命名约定解析出路由参数，一次性生成所有内容页。Header 组件接收 `navPages` prop（构建时从集合计算），导航链接随内容文件自动增减，无需手动维护。
...
内容页（`[slug].astro`）：Header + Markdown 正文 + Footer，正文通过 Astro 内容集合渲染。页面 `<title>` 取自 Markdown frontmatter 的 `title` 字段。
